...
#
# Sets up a consistent baseline TOOLSET across machines for workshops and labs.
# This is NOT a version-locked / reproducible installer: apt pulls whatever Kali
# currently ships, so two boxes get the same tools, not necessarily same versions.
# `verify` enforces a minimum-version FLOOR, not an exact pin.
#
# Installer routing per tool (REGISTRY[*]["installer"]):
#   apt  -> prebuilt Kali package (default, fastest, preferred)
#   pip  -> installed via PIPX into an isolated venv, binary symlinked to
#           /usr/local/bin. NOTE: pipx isolation means these tools are usable as
#           CLI commands but their Python libraries are NOT importable system-wide.
#   go   -> built from source with `go install` (kerbrute only; requires Go).
#
# Usage:
#   sudo python3 classroomenv.py                 interactive menu
#   sudo python3 classroomenv.py install <prof>  install a profile
#   sudo python3 classroomenv.py verify  <prof>  verify a profile against floors
#   sudo python3 classroomenv.py doctor          system health check
#   sudo python3 classroomenv.py inventory       list installed tools + versions
#   sudo python3 classroomenv.py update          apt upgrade + re-verify
#   sudo python3 classroomenv.py fresh           full new-VM bootstrap
#
# Requires root (drives apt). Profiles: core web osint ad wireless exploitation extras full
#
...
REGISTRY = {
    ...
    "golang": {
        ...
        "version_re": r"go(\d+\.\d+[\.\d]*)", "min_version": None,
    ...
    "httpx": {
        "installer": "apt", "package": "httpx-toolkit", "bin": "httpx-toolkit",
        "version_cmd": ["httpx-toolkit", "-version"],
        "version_re": r"v?(\d+\.\d+\.\d+)", "min_version": None,
        "desc": "Fast HTTP probing tool (run as 'httpx-toolkit' on Kali)",
    },
    "nikto": {
        "installer": "apt", "package": "nikto", "bin": "nikto",
        "version_cmd": ["nikto", "-Version"],
        "version_re": r"(\d+\.\d+[\.\d]*)", "min_version": None,
    ...
    "whatweb": {
        ...
        "version_re": r"WhatWeb version (\d+\.\d+[\.\d]*)", "min_version": None,
        "desc": "Web technology fingerprinter",
    },
    "nuclei": {
        "installer": "apt", "package": "nuclei", "bin": "nuclei",
    ...
    "amass": {
        ...
        "version_re": r"v?(\d+\.\d+[\.\d]*)", "min_version": "3.23.3",
        "desc": "Attack surface mapping",
    },
    "subfinder": {
        "installer": "apt", "package": "subfinder", "bin": "subfinder",
    ...
    "dnsx": {
        "installer": "apt", "package": "dnsx", "bin": "dnsx",
    ...
    "shodan": {
        "installer": "pip", "package": "shodan", "bin": "shodan",
        "version_cmd": None,
        "version_re": None, "min_version": None,
        "desc": "Shodan CLI client (library not importable system-wide; pipx-isolated)",
    },
    "exiftool": {
    ...
    "impacket": {
        ...
        "version_re": r"(\d+\.\d+[\.\d]*)", "min_version": "0.11.0",
    ...
    "kerbrute": {
        ...
        "version_re": r"v?(\d+\.\d+\.\d+)", "min_version": "1.0.3",
    ...
    "aircrack-ng": {
        ...
        "version_re": r"Aircrack-ng (\d+\.\d+[\.\d]*)", "min_version": "1.7",
    ...
    "hashcat": {
        ...
        "version_re": r"v?(\d+\.\d+[\.\d]*)", "min_version": "6.2.6",
    ...
    "john": {
        ...
        "version_re": r"John the Ripper[\w\s]*?(\d+\.\d+[\.\d\w]*)", "min_version": None,
        "desc": "John the Ripper password cracker",
    },
    "hydra": {
        "installer": "apt", "package": "hydra", "bin": "hydra",
        "version_cmd": ["hydra", "-h"],
        "version_re": r"Hydra v(\d+\.\d+[\.\d]*)", "min_version": None,
    ...
    "radare2": {
        ...
        "version_re": r"radare2 (\d+\.\d+[\.\d]*)", "min_version": "5.8.8",
    ...
    "gdb": {
        ...
        "version_re": r"GNU gdb[^\d]*(\d+\.\d+[\.\d]*)", "min_version": None,
        "desc": "GNU debugger",
    },
    "strace": {
        "installer": "apt", "package": "strace", "bin": "strace",
        "version_cmd": ["strace", "--version"],
        "version_re": r"strace -- version (\d+\.\d+[\.\d]*)", "min_version": None,
...
def _ensure_pipx():
    ...
    rc, _, _ = _run(["apt-get", "install", "-y", "pipx"], capture=False)
    return rc == 0 and shutil.which("pipx") is not None

def _pip_install(package):
    if not _ensure_pipx():
        fail("pipx unavailable and could not be installed via apt")
        log.error(f"pipx bootstrap failed before installing {package}")
        return False
    env = os.environ.copy()
    env["PIPX_HOME"]    = "/opt/pipx"
    env["PIPX_BIN_DIR"] = "/usr/local/bin"
    try:
        result = subprocess.run(["pipx", "install", "--force", package], env=env, timeout=300)
        if result.returncode != 0:
            return False
        # Python 3.12+ no longer ships setuptools/pkg_resources by default, and
        # several of these tools still import pkg_resources at runtime. Inject
        # setuptools into the tool's isolated venv so it doesn't crash on launch.
        # PIN <81: setuptools 81 removed pkg_resources entirely, so a bare
        # "setuptools" inject pulls 82+ and the tool still crashes. <81 is the
        # last line that still ships pkg_resources.
        app = package.split("==")[0].split(">=")[0].strip()
        inj = subprocess.run(["pipx", "inject", app, "setuptools<81"],
                             env=env, timeout=120)
        if inj.returncode != 0:
            warn(f"installed {app} but could not inject setuptools; "
                 f"the tool may fail with a pkg_resources error")
        return True
    except subprocess.TimeoutExpired:
        fail(f"pipx install timed out: {package}")
        log.error(f"pipx install timeout: {package}")
        return False
    except Exception as e:
        fail(f"pipx install errored: {e}")
        log.error(f"pipx install error for {package}: {e}")
        return False
...
def _go_install(package):
    ...
    try:
        ...
        fail(f"go install timed out after 300s: {package}")
        log.error(f"go install timeout: {package}")
        return False
    except Exception as e:
        fail(f"go install errored: {e}")
        log.error(f"go install error for {package}: {e}")
        return False
...
def _get_version(tool):
    ...
    # pipx tools install their binary to /usr/local/bin. Check there specifically
    # rather than anywhere on PATH, so a conflicting distro/apt copy of the same
    # command (e.g. /usr/bin/shodan from python3-shodan) doesn't make us think the
    # working pipx copy is already present and skip installing it.
    if meta.get("installer") == "pip":
        b = meta.get("bin")
        return "present" if (b and os.path.exists(f"/usr/local/bin/{b}")) else None
...
def _parse_version(v):
    """Pull the numeric components out of a version string into a list of ints.
    'go1.26.3' -> [1, 26, 3], '2.53.0' -> [2, 53, 0], '9.1' -> [9, 1].
    Non-numeric junk (rc, beta, #dev tags) is ignored for comparison."""
    return [int(x) for x in re.findall(r"\d+", str(v))]

def _version_matches(found, minimum):
    """True if `found` is AT LEAST `minimum`. Newer always passes.

    This is a floor check, not an exact pin. A training box should fail
    verification only when a tool has REGRESSED below a known-good baseline,
    not every time Kali ships a routine update. Compares component-by-component
    with zero-padding so '8.18.0' >= '7.88' and '9.1' >= '9.0' both pass."""
    ...
    if not f or not m:
        ...
        return str(found).startswith(str(minimum))
    n = max(len(f), len(m))
    f += [0] * (n - len(f))
    m += [0] * (n - len(m))
    return f >= m
...
def _do_install(profile_name, no_gui=False):
    ...
    if no_gui:
        gui = [t for t in tools if "(GUI)" in REGISTRY.get(t, {}).get("desc", "")]
        if gui:
            tools = [t for t in tools if t not in gui]
            info(f"--no-gui: skipping {len(gui)} GUI tool(s): {', '.join(gui)}")
...
def _do_verify(profile_name):
    ...
    for tool in tools:
        ...
        if found is None:
            ...
            passed.append(tool)
        elif _version_matches(found, minimum):
            ok(f"{tool:<26} {str(found):<14} >= {minimum}")
            passed.append(tool)
        else:

            fail(f"{tool:<26} BELOW BASELINE  need>={minimum}  found={found}")
            warned.append(tool)
    ...
    if warned:
        warn(f"Below baseline: {len(warned)}  ->  {', '.join(warned)}")
    if missing:
...
def _do_inventory():
    ...
    print()
    fmt = f"  {'Tool':<24} {'Type':<5} {'Min':<10} {'Found':<16} Status"
    ...
    for p in profiles:
        for t in _tools_for_profile(p):
            ...
            shown.add(t)
            meta    = REGISTRY.get(t, {})
            minimum = meta.get("min_version") or "---"
            ...
            if found == "not found":
                ...
                status = f"{C.YELLOW}BELOW{C.RESET}"

            print(f"  {t:<24} {inst:<5} {minimum:<10} {found:<16} {status}")
...
def _show_profile_and_confirm(profile_name):
    ...
    for t in own_tools:
        ...
        pin   = m.get("min_version") or "latest"
        desc  = m.get("desc", "")
        found = _get_version(t)
        tick  = f"{C.GREEN}[installed]{C.RESET}" if found else f"{C.DIM}[not installed]{C.RESET}"
        print(f"  {color}{t:<24}{C.RESET} [{inst:<3}] min:{pin:<10} {C.DIM}{desc}{C.RESET}  {tick}")
...
def main():
    ...
    parser.add_argument("--no-gui", action="store_true",
                         help="Skip tools marked (GUI) in their description "
                              "(burpsuite, zaproxy, maltego, bloodhound, ghidra). "
                              "Used by the Docker images, which are CLI-only.")
    ...
    if cmd == "install":
        if not args.target:
            fail("Usage: classroomenv install <profile> [--no-gui]")
            sys.exit(1)
        _do_install(args.target, no_gui=args.no_gui)
    elif cmd == "verify":
        if not args.target:
