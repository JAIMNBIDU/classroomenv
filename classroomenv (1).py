...
def _fix_apt_sources():
    ...
    try:
        ...
        if "kali-rolling" not in content:
            with open(sources, "a") as f:
                f.write("\n" + kali_repo)
            ok("Added kali-rolling repo to sources.list")
        else:
...
def _do_fresh_setup():
  ...
  [1] Fix apt sources (ensure kali-rolling is configured)
  [2] apt-get update
  [3] apt-get full-upgrade  (full system upgrade)
...
""")
    ...
    steps = [
        ...
        ("Full system upgrade",             lambda: _run(["apt-get", "full-upgrade", "-y"], capture=False)),
