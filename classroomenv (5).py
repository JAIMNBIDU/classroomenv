...
"""
classroomenv.py
my tool for setting up a kali environment fast and the same way every time

how to use:
  sudo python3 classroomenv.py          <- just run this, menu comes up
...
# needs root or apt wont work
if os.geteuid() != 0:
    print("\033[91m[!] classroomenv requires root. Run with sudo.\033[0m")
    sys.exit(1)

# color codes and print helpers - ok/fail/warn/info/step
class C:
    RED     = "\033[91m"
    GREEN   = "\033[92m"
    YELLOW  = "\033[93m"
...
# make log root-only on first create
try:
...
# lockfile so two instances cant run at the same time and break apt
import fcntl
...
def _acquire_lock():
    ...
    try:
        ...
        pass  # if locking fails just move on, not worth crashing over
...
# every tool we know about - installer type, version command, pin, description
REGISTRY = {
...
# profiles group tools together - web depends on core etc
PROFILES = {
    "core": {
...
# state file tracks what got installed so inventory works
def _load_state():
...
def _save_state(state):
    ...
    # lock it down, no reason for other users to read this
...
# wrappers for running commands - apt, pip, go
...
# version detection - runs the version cmd and pulls the number out with regex
...
# resolves profile deps so core always installs before web etc
...
# checks installed versions against what we pinned
...
# sanity check the whole system before doing anything
def _do_doctor():
    head("System Health Check")
    issues = 0

    # check we are on kali
    try:
    ...
    # git is needed for a few things
    if shutil.which("git"): ok("git available")
    else: fail("git missing"); issues += 1

    # check free space, full install needs ~10gb
    stat = os.statvfs("/")
    ...
    # make sure the usual dirs are in PATH
...
# shows everything installed with versions and status
def _do_inventory():
...
# runs when someone just installed kali on a vm and wants everything set up
def _fix_apt_sources():
    sources = "/etc/apt/sources.list"
    # HTTPS enforced — not HTTP — to prevent MITM on repo metadata
    kali_repo = "deb https://http.kali.org/kali kali-rolling main contrib non-free non-free-firmware\n"
    try:
        content = open(sources).read()
        # Also fix any existing HTTP-only kali entries to HTTPS
...
# apt update/upgrade then re-verify everything
def _do_update():
    head("System Update")
    for cmd, label in [
...
# ascii banner at the top of every screen
def _banner():
    os.system("clear")
    art = """
░█████╗░██╗░░░░░░█████╗░░██████╗░██████╗██████╗░░█████╗░░█████╗░███╗░░░███╗███████╗███╗░░██╗██╗░░░██╗
...
# shows the tools in a profile before asking to install
def _show_profile_and_confirm(profile_name):
    ...
    # warn if go isnt installed and this profile has go tools
...
# lets you pick which installed profile to verify
def _menu_verify():
    _banner()
...
# checks everything needed before installing anything
# runs at startup and also from menu [r]

# pull out all go-based tools for the warning messages
...
def _check_requirements(auto=False):
    ...
    # go - needed for httpx, nuclei, subfinder etc
    if shutil.which("go"):
...
# profile order for the numbered menu
PROFILE_KEYS = ["core", "web", "osint", "ad", "wireless", "exploitation", "extras", "full"]

def _interactive_menu():
    # Auto requirements check on every launch
...
# entry point - no args = interactive menu, otherwise run the command directly
def main():
    _acquire_lock()
